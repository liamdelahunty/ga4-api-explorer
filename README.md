...
*   **Monthly Reports Runner:** A standalone script (`run_monthly_reports.py`) that runs a specified report module for each calendar month since the earliest available data for a given GA4 property up to the current month. The outputs are saved to the `output/` directory as text files. To run it:
    ...
    ```
*   **Last Month Reports Runner:** A standalone script (`run_last_month_reports.py`) that runs a curated list of essential reports for the last calendar month across all properties listed in `config/properties.json`. Additionally, it runs the aggregated `all_properties_ai_traffic_report` once at the end. To run it:
    ```bash
